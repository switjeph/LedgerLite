...
export default function Transactions() {
  const { transactions, chartOfAccounts } = useLedger(); // Use global state
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Double-entry bookkeeping system.</p>
        </div>
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-gray-50/50 transition-colors align-top">
                      <td className="px-6 py-4 text-sm text-gray-900 font-medium whitespace-nowrap">
                        {tx.date}
                        <div className="text-xs text-gray-400 font-normal mt-1">{tx.id}</div>
                      </td>
                        ...
                        <div className="space-y-1">
                          {tx.entries.map((entry, idx) => (
                            <div key={idx} className="flex justify-between text-sm max-w-xs">
                              <span className={entry.type === 'credit' ? "pl-4 text-gray-600" : "text-gray-900 font-medium"}>
                                {entry.account}
                              </span>
                              <span className="text-gray-500">
                                {entry.type === 'debit' ? `Dr $${entry.amount}` : `Cr $${entry.amount}`}
                              </span>
                            </div>
                          ))}
                        </div>
